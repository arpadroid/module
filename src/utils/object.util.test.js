...
describe('object.util', () => {
    ...
    describe('mergeObjects', () => {
        ...
        it('should deeply merge nested objects', () => {
            expect(mergeObjects({ a: 1, nested: { x: 1, y: 2 } }, { b: 2, nested: { y: 3, z: 4 } })).toEqual({
                a: 1,
                b: 2,
                nested: { x: 1, y: 3, z: 4 }
            });
        ...
        it('should handle type replacements', () => {
            expect(mergeObjects({ nested: 'string' }, { nested: { x: 1 } })).toEqual({ nested: { x: 1 } });

            expect(mergeObjects({ nested: { x: 1 } }, { nested: 'string' })).toEqual({ nested: 'string' });

            expect(mergeObjects({ arr: [1, 2, 3] }, { arr: [4, 5] })).toEqual({ arr: [4, 5] });
        });

        it('should handle null and undefined values', () => {
            expect(mergeObjects({ a: 1, b: null, c: undefined }, { b: 2, c: 3, d: null })).toEqual({
                a: 1,
                b: 2,
                c: 3,
                d: null
        ...
        describe('strict mode', () => {
            it('should only merge existing properties when strict=true', () => {
                expect(mergeObjects({ a: 1, b: 2 }, { b: 3, c: 4 }, true)).toEqual({ a: 1, b: 3 });

                expect(
                    mergeObjects({ a: 1, nested: { x: 1, y: 2 } }, { b: 2, nested: { y: 3, z: 4 } }, true)
